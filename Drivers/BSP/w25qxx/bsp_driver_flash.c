...
{
	uint8_t  times_to_read = Size / 65535;
	uint16_t remain_to_read = Size % 65535;
	uint8_t stat = 0;
	if(times_to_read == 0)
		stat += W25Qx_Read(pData,ReadAddr,Size);
	else
	{
		while(times_to_read--)
		{
			stat += W25Qx_Read(pData,ReadAddr,65535);
			pData    += 65535;
			ReadAddr += 65535;
		}
		if(remain_to_read != 0)
			stat += W25Qx_Read(pData,ReadAddr,remain_to_read);
	}
	return stat;
}
/**
